...
def check_train_batch_size(model, imgsz=640, amp=True, batch=-1, ch=3):
    """
    ...
    with autocast(enabled=amp):
        return autobatch(deepcopy(model).train(), imgsz, fraction=batch if 0.0 < batch < 1.0 else 0.6, ch=ch)
