    ...
    def __call__(self, labels):
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            if img.shape[-1] > 5:
                ...
                im_hsv_ = cv2.merge(
                    (
                        cv2.LUT(hue_, lut_hue),
                        cv2.LUT(sat_, lut_sat),
                        cv2.LUT(val_, lut_val),
                    )
                )
                img[..., -3:] = cv2.cvtColor(
                    im_hsv, cv2.COLOR_HSV2BGR
                )  # no return needed
                img[..., :-3] = cv2.cvtColor(
                    im_hsv_, cv2.COLOR_HSV2BGR
                )  # no return needed
                labels["img"] = img
            else:
                hue, sat, val = cv2.split(cv2.cvtColor(img, cv2.COLOR_BGR2HSV))
